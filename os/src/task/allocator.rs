...
use crate::{config::{KERNEL_STACK_BASE, KERNEL_STACK_SIZE, PAGE_SIZE, TRAP_CONTEXT_START, USER_STACK_SIZE}, mm::{address::{PhysPageNum, VirtAddr, VirtPageNum}, map_area::MapPermission, memory_set::MemorySet, KERNEL_SPACE}, sync::spin::mutex::SpinMutex, trap::TrapContext};
...
impl TrapContextPageAllocator {
    pub fn alloc(tid: TaskID, memory_set: Arc<SpinMutex<MemorySet>>) -> TrapContextPageGuard{
...
impl TrapContextPageGuard {
    ...
    fn new(tid: TaskID, memory_set: Arc<SpinMutex<MemorySet>>) -> Self {
...
impl UserStackAlloctor {
    pub fn alloc(memory_set: Arc<SpinMutex<MemorySet>>, base: usize, id: usize) -> UserStackGuard{
...
impl UserStackGuard {
    pub fn new(memory_set: Arc<SpinMutex<MemorySet>>, base: usize, id: usize) ->  Self{
...
impl RecycleAllocator {
    pub fn new() -> Self {
        RecycleAllocator {
            ...
            recycled: SpinMutex::new(Vec::new()),
        }
    }
